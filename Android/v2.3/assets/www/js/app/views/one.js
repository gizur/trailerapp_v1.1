...
var ScreenOneView = Stapes.subclass({
    ...
    constructor: function(aUsr, aLog, aLanguage, aWrapper) {
        ...
        this.minLimits = 0;
        this.maxLimits = Config.existingDmgLimit;
        this.tt_list_html = '';
    ...
    bindEventHandlers: function() {
        ...
        var extDmg = function(tp, cur, minLimits, maxLimits) {
        	//alert("inside ext dmg");
        	//alert(tp);alert(cur);alert(minLimits);alert(maxLimits);
            ...
            var success = function(data) {
                ...
                for (var index in tts) {
                    if (tts.hasOwnProperty(index)) {
                       ...
                       var clipped_tt = tts[index].getAll();
                        ...
                        var li_tt = "<li><a id='" + tts[index].get('id') + "' href='javascript:void(0);'>" + tts[index].get('damageposition') + '<br/>' + tts[index].get('damagetype') + "</a></li>";

                        that.tt_list_html += li_tt;
             ...
             that._tt_list.html = that.tt_list_html;
             ...
             window.localStorage.setItem('tt_list', JSON.stringify(that._tt_list));
              ...
              // alert(ttc.size());
              // alert(Config.existingDmgLimit-1);
               if (ttc.size() > (Config.existingDmgLimit-1)) { 
                that.minLimits = minLimits+ttc.size();
                var mes=that._language.translate('View More') ;
               $('#one #troubleticketlist').append("<li><a href='javascript:void(0)' id='vmore'>"+mes+" </a></li>");
               } else {
            	   if(ttc.size()>0) {
            	   $("#existingSuccessDialog").modal("show");
            	   }
        ...
        $(document).off('click',"#vmore").on('click',"#vmore",function(e) { 
        	e.preventDefault();
        	//alert("click on view more");
        	$(this).html("<li>" + that._language.translate('Loading more') + "</li>");
